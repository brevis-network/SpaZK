...
use pcs::{
    bench_templates::{rand_ml_point, rand_ternary_ml_poly},
    commitment::{
...
fn run<PCS: CommitmentScheme<Field = Fr>>(name: &'static str, num_vars: usize, sparsity: usize) {
    ...
    let mat_dense = rand_ternary_ml_poly(2 * num_vars, sparsity, rng);
    ...
    let timer =
        start_timer!(|| format!("{}, {}, 1/{}, ternary prove", name, num_vars, 1 << sparsity));
