...
use crate::{
    field,
    poly::{dense::DenseMultilinearExtension, unipoly::UniPoly},
    utils::{
    ...
    <P as Pairing>::ScalarField: field::JoltField,
{
    ...
    ark_std::cfg_iter_mut!(v).enumerate().for_each(|(i, v_i)| {
        // for each point u
        ark_std::cfg_iter_mut!(v_i).zip_eq(f).for_each(|(v_ij, f)| {
            // for each poly f
    ...
    <P as Pairing>::ScalarField: field::JoltField,
{
    ...
    let q_powers_multiplied: Vec<P::ScalarField> = ark_std::cfg_iter!(q_powers)
        .map(|q_power| *q_power * q_power_multiplier)
        .collect();

    // Compute the batched openings
    // compute B(u_i) = v[i][0] + q*v[i][1] + ... + q^(t-1) * v[i][t-1]
    let B_u = ark_std::cfg_into_iter!(v)
        .map(|v_i| {
            ark_std::cfg_into_iter!(v_i)
                .zip(ark_std::cfg_iter!(q_powers))
                .map(|(a, b)| *a * *b)
                .sum()
        })
    ...
    <P as Pairing>::ScalarField: field::JoltField,
{
    ...
    pub fn commit(
        ...
        if pp.kzg_pk.g1_powers().len() < poly.evaluations.len() {
        ...
        Ok(HyperKZGCommitment(UnivariateKZG::commit(
            &pp.kzg_pk,
            &UniPoly::from_coeff(poly.evaluations.clone()),
        )?))
    }
    ...
    pub fn open(
        ...
        polys.push(poly.evaluations.to_vec());
        for i in 0..ell - 1 {
            let Pi_len = polys[i].len() / 2;
            let mut Pi = vec![P::ScalarField::zero(); Pi_len];

            #[allow(clippy::needless_range_loop)]
            ark_std::cfg_iter_mut!(Pi)
                .enumerate()
                .for_each(|(j, Pi_j)| {
                    *Pi_j = x[i] * (polys[i][2 * j + 1] - polys[i][2 * j]) + polys[i][2 * j];
                });
        ...
        let com: Vec<P::G1Affine> = ark_std::cfg_into_iter!((1..polys.len()))
            .map(|i| {
    ...
    pub fn verify(
        ...
        for i in 0..ell {
            if two * r * Y[i + 1]
                != r * (P::ScalarField::one() - x[i]) * (ypos[i] + yneg[i])
                    + x[i] * (ypos[i] - yneg[i])
            {
                return Err(ProofVerifyError::InternalError);
            }
    ...
    fn batch_open(
        pk: &HyperKZGProverKey<P>,
        polynomials: &[&DenseMultilinearExtension<P::ScalarField>],
        point: &[P::ScalarField],
        evals: &[P::ScalarField],
        transcript: &mut ProofTranscript,
    ) -> HyperKZGProof<P> {
        ...
        let f_batched = {
            let mut f_batched = unsafe_allocate_zero_vec::<P::ScalarField>(n);
            for (coeff, poly) in rho_powers.iter().zip(polynomials.iter()) {
                for (rlc, poly_eval) in f_batched.iter_mut().zip(poly.evals_ref().iter()) {
                    *rlc += mul_0_1_optimized(poly_eval, coeff);
                }
            }
            f_batched
        ...
        let poly = DenseMultilinearExtension::new(f_batched);
    ...
    <P as Pairing>::ScalarField: field::JoltField,
{
    ...
    fn commit(
        ...
        assert!(
            setup.0.kzg_pk.g1_powers().len() > poly.evaluations.len(),
            "COMMIT KEY LENGTH ERROR {}, {}",
            setup.0.kzg_pk.g1_powers().len(),
            poly.evaluations.len()
        );
        HyperKZGCommitment(
            UnivariateKZG::commit(
                &setup.0.kzg_pk,
                &UniPoly::from_coeff(poly.evaluations.clone()),
            )
            .unwrap(),
...
mod tests {
    ...
    fn test_hyperkzg_eval() {
        ...
        let poly = DenseMultilinearExtension::new(vec![
            Fr::from(1),
            Fr::from(2),
            Fr::from(2),
            Fr::from(4),
