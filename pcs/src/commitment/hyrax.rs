...
impl<F: JoltField, G: CurveGroup<ScalarField = F>> HyraxCommitment<G> {
    ...
    pub fn commit_slice(eval_slice: &[G::ScalarField], generators: &PedersenGenerators<G>) -> Self {
        ...
        let row_commitments = ark_std::cfg_chunks!(eval_slice, R_size)
    ...
    pub fn batch_commit(
        ...
        let rows = ark_std::cfg_iter!(batch).flat_map(|poly| ark_std::cfg_chunks!(poly, R_size));
        let row_commitments: Vec<G> = rows
            .map(|row| PedersenCommitment::commit_vector(row, &gens))
            .collect();

        ark_std::cfg_chunks!(row_commitments, L_size)
            .map(|chunk| Self {
...
impl<F: JoltField, G: CurveGroup<ScalarField = F>> HyraxOpeningProof<G> {
    ...
    pub fn prove(
        ...
        assert_eq!(poly.num_vars, opening_point.len());

        // compute the L and R vectors
        let (L_size, R_size) = matrix_dimensions(poly.num_vars, ratio);
        let eq = EqPolynomial::new(opening_point.to_vec());
        let (L, _R) = eq.compute_factored_evals(R_size);
    ...
    pub fn verify(
        ...
        let (L, R) = eq.compute_factored_evals(R_size);
    ...
    fn vector_matrix_product(
        ...
        let (_, R_size) = matrix_dimensions(poly.num_vars, ratio);
        ...
        #[cfg(not(feature = "parallel"))]
        {
            poly.evals_ref()
                ...
                .fold(vec![G::ScalarField::zero(); R_size], |mut acc, row| {
                    acc.iter_mut().zip(row).for_each(|(x, y)| *x += y);
                    acc
                })
        }
...
impl<F: JoltField, G: CurveGroup<ScalarField = F>> BatchedHyraxOpeningProof<G> {
    ...
    pub fn prove(
        polynomials: &[&DenseMultilinearExtension<G::ScalarField>],
        ...
        let poly_len = polynomials[0].evaluations.len();
        ...
        let rlc_poly = {
            ...
            if chunk_size > 0 {
                ...
                rlc_coefficients
                    ...
                    .map(|(coeff, poly)| {
                        poly.evals_ref().iter().map(|eval| *coeff * *eval).collect()
                    })
                    .reduce(
        ...
        let rlc_poly = {
            rlc_coefficients
                ...
                .map(|(coeff, poly)| {
                    poly.evals_ref()
                        .iter()
                        .map(|eval| mul_0_1_optimized(coeff, eval))
                        .collect::<Vec<G::ScalarField>>()
                })
                .fold(vec![G::ScalarField::zero(); poly_len], |mut acc, row| {
                    acc.iter_mut().zip(row).for_each(|(x, y)| *x += y);
                    acc
                })
        };
        ...
        let joint_proof = HyraxOpeningProof::prove(
            &DenseMultilinearExtension::new(rlc_poly),
            opening_point,
            ratio,
            transcript,
        );
    ...
    pub fn verify(
        ...
        let rlc_commitment = rlc_coefficients
            ...
            .fold(vec![G::zero(); L_size], |running, new| {
                debug_assert_eq!(running.len(), new.len());
                running
                    .iter()
                    .zip(new.iter())
...
mod tests {
    ...
    >() {
        ...
        let poly = DenseMultilinearExtension::new(Z);
