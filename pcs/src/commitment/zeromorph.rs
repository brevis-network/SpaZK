...
{
    let num_var = poly.num_vars;
    assert_eq!(num_var, point.len());

    let mut remainder = poly.evaluations.to_vec();
    let mut quotients: Vec<_> = point
        ...
        .map(|(i, x_i)| {
            ...
            ark_std::cfg_iter_mut!(remainder_lo)
                .zip(remainder_hi)
                .for_each(|(r_lo, r_hi)| {
                    *r_lo += (*r_hi - *r_lo) * *x_i;
                });
    ...
    <P as Pairing>::ScalarField: JoltField,
{
    ...
    let q_hat = quotients.iter().enumerate().fold(
        ...
        |mut q_hat, (idx, q)| {
            let q_hat_iter = ark_std::cfg_iter_mut!(q_hat[(1 << num_vars) - (1 << idx)..]);
    ...
    <P as Pairing>::ScalarField: JoltField,
{
    ...
    pub fn commit(
        ...
        if pp.commit_pp.g1_powers().len() < poly.evaluations.len() {
    ...
    pub fn open(
        ...
        let q_k_com: Vec<P::G1Affine> = ark_std::cfg_iter!(quotients)
            .map(|q| UnivariateKZG::commit(&pp.commit_pp, q).unwrap())
            .collect();
        let q_comms: Vec<P::G1> = ark_std::cfg_iter!(q_k_com)
            .map(|c| c.into_group())
            .collect();
        ...
        let mut f = UniPoly::from_coeff(poly.evaluations.clone());
        f *= &z_challenge;
        f += &q_hat;
    ...
    <P as Pairing>::ScalarField: JoltField,
{
    ...
    fn commit(
        ...
        assert!(
            setup.0.commit_pp.g1_powers().len() > poly.evaluations.len(),
    ...
    fn partially_evaluated_quotient_z_x() {
        ...
        for k in 0..num_vars {
            ...
            let mut scalar = x_pow_2k * phi::<Bn254>(&x_pow_2kp1, num_vars - k - 1)
                - challenges[k] * phi::<Bn254>(&x_pow_2k, num_vars - k);
            scalar *= z_challenge;
            scalar *= Fr::from(-1);
