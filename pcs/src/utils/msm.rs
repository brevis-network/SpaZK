...
pub trait VariableBaseMSM: ScalarMul {
    fn msm(bases: &[Self::MulBase], scalars: &[Self::ScalarField]) -> Result<Self, usize> {
        (bases.len() == scalars.len())
            .then(|| {
                #[cfg(feature = "parallel")]
                let abs_max_num_bits = scalars
                    .par_iter()
                    .map(|s| {
                        let neg_s = -*s;
                        s.into_bigint()
                            .num_bits()
                            .min(neg_s.into_bigint().num_bits())
                    })
                    .max()
                    .unwrap();
                ...
                if abs_max_num_bits <= 10 {
                    max_num_bits = abs_max_num_bits;
                }

                match max_num_bits {
                    0 => Self::zero(),
                    1 => {
                        let scalars_u64 = &map_ternary_elements_to_i64::<Self>(scalars);
                        msm_binary(bases, scalars_u64)
                    }
                    2..=10 => {
                        let scalars_i64 = &map_field_elements_to_i64::<Self>(scalars);
                        msm_small(bases, scalars_i64, max_num_bits as usize)
                    }
                    11..=64 => {
                    ...
                    _ => {
                        ...
                        if Self::NEGATION_IS_CHEAP {
                            msm_bigint_wnaf(bases, scalars.as_slice(), max_num_bits as usize)
                        } else {
                            msm_bigint(bases, scalars.as_slice(), max_num_bits as usize)
                        }
                    }
                }
            })
...
fn map_field_elements_to_u64<V: VariableBaseMSM>(field_elements: &[V::ScalarField]) -> Vec<u64> {
    ark_std::cfg_iter!(field_elements)
        .map(|s| {
...
fn map_field_elements_to_i64<V: VariableBaseMSM>(field_elements: &[V::ScalarField]) -> Vec<i64> {
    ark_std::cfg_iter!(field_elements)
        .map(|s| {
            let bigint = s.into_bigint();
            let neg_big_int = (-*s).into_bigint();
            if bigint < neg_big_int {
                let limbs: &[u64] = bigint.as_ref();
                limbs[0] as i64
            } else {
                let limbs: &[u64] = neg_big_int.as_ref();
                -(limbs[0] as i64)
            }
        })
        .collect::<Vec<_>>()
...
fn map_ternary_elements_to_i64<V: VariableBaseMSM>(field_elements: &[V::ScalarField]) -> Vec<i64> {
    let one = V::ScalarField::one();
    ark_std::cfg_iter!(field_elements)
        .map(|s| {
            if *s == one {
                1
...
fn msm_bigint_wnaf<V: VariableBaseMSM>(
    ...
    let scalar_digits = scalars
        .into_iter()
        .flat_map(|s| make_digits_bigint(s, c, num_bits))
        .collect::<Vec<_>>();
    let zero = V::zero();
...
fn msm_u64_wnaf<V: VariableBaseMSM>(
    ...
    let scalar_digits = scalars
        .into_iter()
        .flat_map(|s| make_digits_u64(*s, c, max_num_bits))
        .collect::<Vec<_>>();
    let zero = V::zero();
...
fn msm_u64<V: VariableBaseMSM>(bases: &[V::MulBase], scalars: &[u64], max_num_bits: usize) -> V {
    ...
    let window_starts = ark_std::cfg_into_iter!((0..max_num_bits)).step_by(c);
...
fn msm_binary<V: VariableBaseMSM>(bases: &[V::MulBase], scalars: &[i64]) -> V {
    scalars
        .iter()
        .zip(bases)
        .filter(|(&scalar, _base)| scalar != 0)
        .map(|(scalar, base)| (*scalar == 1, base))
        .fold(
            V::zero(),
            |sum, base| if base.0 { sum + base.1 } else { sum - base.1 },
        )
}

#[tracing::instrument(skip_all, name = "msm_small")]
fn msm_small<V: VariableBaseMSM>(bases: &[V::MulBase], scalars: &[i64], max_num_bits: usize) -> V {
