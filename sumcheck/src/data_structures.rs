...
#[derive(Clone)]
pub struct ListOfProductsOfPolynomials<F: JoltField> {
...
impl<F: JoltField> ListOfProductsOfPolynomials<F> {
    ...
    pub fn expected_sum(&self) -> F {
        let mut sum = F::zero();
        let size = 1 << self.num_variables;
        for (c, p) in &self.products {
            for i in 0..size {
                let mut product = F::one();
                for &j in p {
                    product *= self.flattened_ml_extensions[j].evaluations[i];
                }
                sum += *c * product;
            }
        }
