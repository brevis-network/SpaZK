...
pub struct ProverMsg<F: JoltField> {
    /// evaluations on P(0), P(1), P(2), ...
    pub evaluations: Vec<F>,
}
/// Prover State
pub struct ProverState<F: JoltField> {
...
impl<F: JoltField> IPForMLSumcheck<F> {
    ...
    pub fn prove_round(
        ...
        let zeros = (vec![F::zero(); degree], vec![F::zero(); degree]);
        #[cfg(feature = "parallel")]
        let zeros = || (vec![F::zero(); degree], vec![F::zero(); degree]);
...
impl<F: JoltField> AppendToTranscript for ProverMsg<F> {
    fn append_to_transcript(&self, transcript: &mut ProofTranscript) {
        transcript.append_scalars(&self.evaluations);
    }
}

impl<F: JoltField> ProverState<F> {
    /// Get the final evaluations of some polynomial.
    pub fn final_evaluation(&self, poly_idx: usize) -> F {
        let poly = &self.flattened_ml_extensions[poly_idx];
        assert_eq!(poly.num_vars, 1);
        poly.fix_variables(&self.randomness[&self.randomness.len() - 1..])[0]
    }
}
