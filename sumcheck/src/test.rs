use crate::{data_structures::ListOfProductsOfPolynomials, protocol::IPForMLSumcheck, MLSumcheck};
use ark_bn254::Fr;
use ark_std::{
    rand::{Rng, RngCore},
    rc::Rc,
...
use pcs::{
    field::JoltField, poly::dense::DenseMultilinearExtension, utils::transcript::ProofTranscript,
};

fn random_product<F: JoltField, R: RngCore>(
    nv: usize,
...
fn random_list_of_products<F: JoltField, R: RngCore>(
    nv: usize,
...
fn test_protocol(nv: usize, num_multiplicands_range: (usize, usize), num_products: usize) {
    ...
    for _ in 0..poly.num_variables {
        ...
        let verifier_msg2 =
            IPForMLSumcheck::verify_round(prover_message, &mut verifier_state, &mut transcript);
...
fn test_polynomial_as_subprotocol(
    ...
    let (proof, prover_state) =
        MLSumcheck::prove_as_subprotocol(prover_transcript, &poly).expect("fail to prove");
    let subclaim =
        MLSumcheck::verify_as_subprotocol(verifier_transcript, &poly_info, asserted_sum, &proof)
...
fn test_normal_polynomial_different_transcripts_fails() {
    ...
    let mut prover_transcript = ProofTranscript::new(b"test");
    let mut verifier_transcript = ProofTranscript::new(b"test fail");
    test_polynomial_as_subprotocol(
        nv,
        num_multiplicands_range,
        num_products,
...
fn test_shared_reference() {
    ...
    poly.add_product(vec![ml_extensions[4].clone()], Fr::random(&mut rng));
    ...
    let poly_info = poly.info();
    let proof = MLSumcheck::prove(&poly).expect("fail to prove");
    let asserted_sum = poly.expected_sum();
    let subclaim = MLSumcheck::verify(&poly_info, asserted_sum, &proof).expect("fail to verify");
    assert!(
        poly.evaluate(&subclaim.point) == subclaim.expected_evaluation,
        "wrong subclaim"
